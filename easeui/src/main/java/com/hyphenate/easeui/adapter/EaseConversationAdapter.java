...
public class EaseConversationAdapter extends ArrayAdapter<EMConversation> {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (conversation.getType() == EMConversationType.GroupChat) {
            ...
            EMMessage lastMsg = conversation.getLatestMessageFromOthers();

            String nickName;
            String headImg;
           if(lastMsg == null){
               lastMsg = conversation.getLastMessage();
               nickName = lastMsg.getStringAttribute("sendUserName", "未获取");
               headImg = lastMsg.getStringAttribute("sendHeadImg",null);
               Glide.with(getContext()).load(headImg).into(holder.avatar);
               EaseUserUtils.setUserNick(nickName, holder.name);
           }else{
               nickName = lastMsg.getStringAttribute("userName", "未获取");
               headImg = lastMsg.getStringAttribute("headImg",null);
               Glide.with(getContext()).load(headImg).into(holder.avatar);
               EaseUserUtils.setUserNick(nickName, holder.name);
           }
