...
public class EaseChatFragment extends EaseBaseFragment implements EMMessageListener {
    ...
    protected static final int REQUEST_CODE_HOUSE = 4;
...
//    static final int ITEM_LOCATION = 3;
    static final int ITEM_HOUSE = 3;
    protected int[] itemStrings = { R.string.attach_take_pic, R.string.attach_picture, R.string.attach_house};
    protected int[] itemdrawables = { R.drawable.ease_chat_takepic_selector, R.drawable.ease_chat_image_selector,R.drawable.fangyuan};
    protected int[] itemIds = { ITEM_TAKE_PICTURE, ITEM_PICTURE,ITEM_HOUSE };
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        user_head = fragmentArgs.getString(EaseConstant.USER_HEAD);
        my_head = fragmentArgs.getString(EaseConstant.MY_HEAD);
        my_name = fragmentArgs.getString(EaseConstant.MY_NAME);
        my_id = fragmentArgs.getString(EaseConstant.My_ID);

        user_head = fragmentArgs.getString(EaseConstant.USER_HEAD);
        user_id = fragmentArgs.getString(EaseConstant.USER_ID);
        user_name = fragmentArgs.getString(EaseConstant.USER_NAME);
...
//        messageList.setAvatarShape(1);
        ...
        if(conversation == null){
            sendTextMessage("Hi，我是您的经纪人"+my_name+"，很高兴为您提供服务，您有任何租房需求和问题欢迎向我咨询。");
        }
        if(fragmentArgs.getString(EaseConstant.HOUSE_ID)!= null){
            if(!fragmentArgs.getString(EaseConstant.HOUSE_ID).equals("")){
                EMMessage message = EMMessage.createTxtSendMessage(" ", toChatUsername);
                message.setAttribute("dataType","102");
                message.setAttribute("houseId",fragmentArgs.getString(EaseConstant.HOUSE_ID));
                message.setAttribute("roomId",fragmentArgs.getString(EaseConstant.ROOM_ID));
                message.setAttribute("type",fragmentArgs.getString(EaseConstant.HOUSE_TYPE));
                message.setAttribute("houseTitle",fragmentArgs.getString(EaseConstant.HOUSE_TITLE));
                message.setAttribute("houseImg",fragmentArgs.getString(EaseConstant.HOUSE_PHOTO));
                message.setAttribute("housePrice",fragmentArgs.getString(EaseConstant.HOUSE_RENTAL));
                sendMessage(message);
            }
        }
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == Activity.RESULT_OK) {
            if (requestCode == REQUEST_CODE_CAMERA) { // capture new image
                Log.d("dfdasfa","cameraFile"+cameraFile);
            ...
            }else if(requestCode == REQUEST_CODE_HOUSE){
                sendHouseMessage(data);
//                sendTextMessage(data.getStringExtra(EaseConstant.HOUSE_TITLE));
...
//                selectPicFromCamera();
            ...
            case ITEM_HOUSE:
                ...
                Intent intent = new Intent("com.fdm.chathouse");
                int requestCode = REQUEST_CODE_HOUSE;
    ...
    protected void sendHouseMessage(Intent intent){
        EMMessage message = EMMessage.createTxtSendMessage(" ", toChatUsername);
        message.setAttribute("dataType","102");
        message.setAttribute("houseId",intent.getStringExtra(EaseConstant.HOUSE_ID));
        message.setAttribute("roomId",intent.getStringExtra(EaseConstant.ROOM_ID));
        message.setAttribute("type",intent.getStringExtra(EaseConstant.HOUSE_TYPE));
        message.setAttribute("houseTitle",intent.getStringExtra(EaseConstant.HOUSE_TITLE));
        message.setAttribute("houseImg",intent.getStringExtra(EaseConstant.HOUSE_PHOTO));
        message.setAttribute("housePrice",intent.getStringExtra(EaseConstant.HOUSE_RENTAL));
        sendMessage(message);
    }
...
//        headImg    —>   头像
//        userId —> 当前自己的id
        message.setAttribute("userName",my_name);
        message.setAttribute("headImg",my_head);
        message.setAttribute("userId",my_id);

        Log.d("11111111111","nickName+"+user_name);

        message.setAttribute("sendUserName",user_name);
        message.setAttribute("sendHeadImg",user_head);
        message.setAttribute("sendUserId",user_id);
    ...
    private void openCamra() {
        ...
        cameraFile = new File(Environment
        ...
        openCameraIntent.putExtra(MediaStore.EXTRA_OUTPUT, cameraFile);
