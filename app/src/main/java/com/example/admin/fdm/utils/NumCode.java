...
public class NumCode {
    ...
    public static NumCode getInstance() {
        if (bpUtil == null)
            bpUtil = new NumCode();
        return bpUtil;
    }
    ...
    private static final int BASE_PADDING_TOP = 21;//上边距
    private static final int RANGE_PADDING_TOP = 7;//上边距范围值
    private static final int DEFAULT_WIDTH = 100;//默认宽度.图片的总宽
    private static final int DEFAULT_HEIGHT = 40;//默认高度.图片的总高
    private final int DEFAULT_COLOR = Color.WHITE;//默认背景颜色值
    ...
    private Bitmap createBitmap() {
        ...
        Bitmap bp = Bitmap.createBitmap(width, height, Config.ARGB_8888); //创建空白图形
        Canvas c = new Canvas(bp);//画板
        ...
        for (int i = 0; i < code.length(); i++) {
            ...
            randomPadding();
            c.drawText(code.charAt(i) + "", padding_left, padding_top, paint);//画画 写字
        }
